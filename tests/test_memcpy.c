...
/*   Updated: 2025/10/28 13:33:23 by dloic            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "libtest.h"
#include <string.h>

int run_memcpy_tests(int count, t_test_unit *tests)
{
	int     i;
	int     correct;
...
//		ft_memcpy(tests[i].dest, tests[i].src, tests[i].n);
		memcpy(tests[i].dest, tests[i].src, tests[i].n);
...
int create_memcpy_tests(void)
{
	...
	return (run_memcpy_tests(count, tests) == count);
}
